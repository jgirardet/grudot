...
import { EXTENSION_ID } from "../../constantes";
export function get_extension_uri(...paths: string[]) {
  return Uri.joinPath(
    extensions.getExtension(EXTENSION_ID)!.extensionUri,
    ...(paths ?? "")
  );
}

const iconDir = get_extension_uri("src", "godotVscode", "godot_icons").fsPath;
  ...
  public parse_body() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith("script = ExtResource")) {
        this.hasScript = true;
        this.scriptId = line.match(
          /script = ExtResource\(\s*"?([\w]+)"?\s*\)/
        )![1]; // should be ok
...
export class Scene {
  ...
  constructor(path: string, title: string, mtime: number) {
    (this.path = path), (this.title = title), (this.mtime = mtime);
  }
}
